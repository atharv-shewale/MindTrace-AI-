...
const BreathingSession = ({ onBack }) => {
  ...
  const phases = {
    ...
    'hold-out': { label: 'Rest', color: 'text-gray-400', bg: 'bg-gray-500/10', scale: 'scale-100' },
  };
    ...
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl">
            <span className="text-gray-500 text-[10px] font-black tracking-widest uppercase mr-2">Cycles:</span>
            <span className="text-white font-bold">{cycles}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center space-y-16 py-12">
        <div className="relative flex items-center justify-center">
          ...
          <div className={`absolute w-80 h-80 border border-white/5 rounded-full transition-all duration-[4000ms] ease-in-out ${isActive ? phases[phase].scale : 'scale-75'}`}></div>
          
          {/* Main Circle */}
          <div className={`w-48 h-48 rounded-full border-2 border-white/10 flex items-center justify-center bg-[#000]/40 backdrop-blur-xl z-10 transition-all duration-[4000ms] ease-in-out ${isActive ? phases[phase].scale : 'scale-90'}`}>
            <div className="text-center">
              <h2 className={`text-2xl font-black tracking-tighter uppercase mb-1 ${phases[phase].color}`}>{phases[phase].label}</h2>
              <p className="text-4xl font-black text-white">{seconds}</p>
            </div>
          </div>
        ...
        <div className="text-center space-y-8">
          ...
          <div className="flex items-center justify-center gap-4">
            <button
              ...
              className="p-4 bg-white/5 border border-white/10 rounded-2xl text-gray-400 hover:text-white hover:border-white/20 transition-all"
            >
            ...
            <button
              ...
              className={`w-20 h-20 rounded-full flex items-center justify-center transition-all ${
                isActive 
                  ...
                  : 'bg-indigo-600 text-white shadow-[0_0_30px_rgba(99,102,241,0.4)] hover:bg-indigo-500'
              }`}
            >
